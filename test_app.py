...
def test_get_course_by_id_exists():
    response = client.get("/courses/663b9f57f518699fc8bbd28a")
    assert response.status_code == 200
    course = response.json()
    # get the course from the database
    course_db = db.courses.find_one({'_id': ObjectId('663b9f57f518699fc8bbd28a')})
...
def test_get_chapter_info():
    response = client.get("/courses/663b9f57f518699fc8bbd28a/1")
    assert response.status_code == 200
...
def test_get_chapter_info_not_exists():
    response = client.get("/courses/663b9f57f518699fc8bbd28a/990")
    assert response.status_code == 404
...
def test_rate_chapter():
    course_id = "663b9f57f518699fc8bbd28a"
    chapter_id = "1"
    rating = 1
...
def test_rate_chapter_not_exists():
    response = client.post("/courses/663b48923b71b2b16eaeb3f3/990/rate", json={"rating": 1})
    assert response.status_code == 404
